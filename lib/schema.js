...
const excludeTables = (tables = [], omittedPrefixes = []) => {
    ...
    return tables
        .map((t)=>t.table_name)
        .filter((name)=> omittedPrefixes.indexOf((name.split('_')[0] || name) < 0));
};
...
module.exports = function (db, namespace, omittedPrefixes) {
    ...
    return function *() {
        ...
        return excludeTables(results.tables, omittedPrefixes)
            .reduce(updateEntitiesWithTable(results), {});
    };
};
